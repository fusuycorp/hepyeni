# HepYeni Architecture Guide

This document outlines the core architectural patterns, execution lifecycles, and security mechanisms of the **HepYeni** platform.

---

## 1. High-Level System Architecture

HepYeni uses a **Next.js 15+ (React 19) App Router** frontend running on the **Bun** runtime, paired with **PocketBase** (embedded SQLite) as a decoupled headless backend and authentication engine.
...
**HepYeni eliminates client-side database access entirely:**
1. All PocketBase collections enforce `null` API rules (`listRule`, `viewRule`, `createRule`, `updateRule`, `deleteRule`).
2. Only the Next.js server executes database operations using an authenticated **Superuser Client** ([`src/lib/pocketbase/superuser.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/pocketbase/superuser.ts)).
3. All UI data mutations occur via Next.js Server Actions ([`src/lib/actions/*`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/)).
...
Used for standard form submissions (e.g., [`voteOnTitle`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/votes.ts), [`markConsumed`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/titles.ts), [`submitReview`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/reviews.ts)).
- Next.js executes the action on the server.
- The action calls [`revalidatePath()`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/votes.ts#L58) to refresh the Server Component cache.
...
Used for dialogs, search inputs, and modal flows (e.g., [`searchTitles`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/titles.ts#L22), [`addTitle`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/titles.ts#L34), [`createGroup`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/groups.ts#L28), [`leaveGroup`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/groups.ts#L160), [`deleteAccount`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/profile.ts#L22)).
...
Located in [`src/lib/pocketbase/superuser.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/pocketbase/superuser.ts), this module manages the authoritative backend connection:
...
2. **Concurrent Auth Mutex**: If the superuser token expires or is uninitialized, [`ensureAuthenticated()`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/pocketbase/superuser.ts#L24-L40) sets a shared `globalThis.__pbSuperuserAuth` promise. All concurrent requests await the single authentication attempt without issuing redundant superuser login calls.
...
Authentication is managed in [`src/lib/pocketbase/session.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/pocketbase/session.ts):
...
2. **On every incoming request**, [`getSessionFromToken()`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/pocketbase/session.ts#L21-L43) calls:
   ```typescript
...
- By calling `authRefresh()` on PocketBase server-side, HepYeni verifies the cryptographic signature, checks user validity, and retrieves the latest database record (`bannedAt`, `isAdmin`, `name`).
- If an admin bans a user via [`banUser()`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/admin.ts#L33), their session is rejected on their **very next HTTP request** without maintaining a separate Redis/memory session store.

---

## 5. Edge Proxy Routing & Auth Gate

Defined in [`src/proxy.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/proxy.ts):
...
PocketBase record IDs are capped at 15 lowercase alphanumeric characters. Rather than generating random IDs, HepYeni derives a deterministic 15-character base36 hash of `titleId:userId` in [`src/lib/pocketbase/vote-id.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/pocketbase/vote-id.ts):
...
3. The catch block in [`src/lib/actions/votes.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/actions/votes.ts#L39-L56) intercepts the 400 error:
...
Helpers are centralized in [`src/lib/membership.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/membership.ts) and [`src/lib/admin.ts`](file:///home/devhax/projects/fusuycorp/hepyeni/src/lib/admin.ts).

---

## 8. Responsive Design & Layout Hierarchy

- **Desktop (`≥768px`)**: Renders [`DesktopSidebar`](file:///home/devhax/projects/fusuycorp/hepyeni/src/components/layout/desktop-sidebar.tsx) containing circle shortcuts, activity links, admin access (for admins), user card, and dark/light theme switch. Group details render in a 3-column split view (2 columns for media backlogs, 1 column for circle info and member rosters).
- **Mobile (`<768px`)**: Renders a compact top bar and a fixed bottom navigation bar ([`BottomNav`](file:///home/devhax/projects/fusuycorp/hepyeni/src/components/bottom-nav.tsx)) with minimum 44px touch targets and safe-area inset padding.
- **Media Presentation**: Handled by [`MediaCover`](file:///home/devhax/projects/fusuycorp/hepyeni/src/components/media-cover.tsx) with a strict `aspect-[2/3]` container to prevent Cumulative Layout Shift (CLS) across third-party image domains.
