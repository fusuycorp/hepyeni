...
export default async function LoginPage({
  ...
  searchParams: Promise<{ error?: string; step?: string; notice?: string; trace?: string }>;
}) {
  const session = await getSession();
  if (session) redirect("/groups");

  const { error, step, notice, trace } = await searchParams;
  const [otp, t, authProviders] = await Promise.all([
    step === "code" ? peekOtpCookie() : Promise.resolve(null),
  ...
  const ERROR_MESSAGES: Record<string, string> = {
    ...
    SignupFailed: t.auth.errors.signup_failed,
    OAuthNotConfigured: t.auth.errors.oauth_not_configured,
    OAuthFailed: t.auth.errors.oauth_failed,
  };

  const NOTICE_MESSAGES: Record<string, string> = {
    ResetComplete: t.auth.notices.password_reset_success,
  };
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        {error && ERROR_MESSAGES[error] && (
          <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-xs">
            ...
            {trace && (
              <span className="font-mono text-[10px] opacity-80 pl-6 select-all">
                {t.diagnostics.traceId}: {trace}
              </span>
            )}
          </div>
        )}
            ...
            {otp ? (
              <form action={verifyEmailCode} className="space-y-4 text-center">
                <div className="space-y-1">
                  ...
                  <h2 className="text-sm font-semibold">{t.auth.verifyCodeTitle}</h2>
                  <p className="text-xs text-muted-foreground">
                    {t.auth.otpCodeSentTo.split("{email}")[0]}
                    <span className="font-medium text-foreground">{otp.email}</span>
                    {t.auth.otpCodeSentTo.split("{email}")[1]}
                  </p>
                </div>
              ...
              <div className="space-y-4">
                {/* Social Login Buttons (Dynamic based on PocketBase active providers) */}
                ...
                <Tabs defaultValue="link" className="w-full">
                  <TabsList className="grid grid-cols-2 w-full mb-3">
                    <TabsTrigger value="link" className="text-xs">{t.auth.tabEmailOtp}</TabsTrigger>
                    <TabsTrigger value="password" className="text-xs">{t.auth.tabPassword}</TabsTrigger>
                  </TabsList>
                        ...
                        <DialogContent className="max-h-[85dvh] overflow-y-auto">
                          <DialogHeader>
                            ...
                            <DialogDescription className="text-xs">
                              {t.auth.forgotPasswordDialogDesc}
                            </DialogDescription>
                          </DialogHeader>
                          <ForgotPasswordForm />
                        </DialogContent>
