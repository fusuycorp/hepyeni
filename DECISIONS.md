...
## ADR-014: LLM Text Extraction Pipeline, Untrusted Ingestion Boundary & Durable Reservation Quotas
- **Status**: Accepted & Implemented (2026-08-19)
- **Context**: Users frequently discover media suggestions from unstructured chat messages, social media posts, or review lists. Manually searching and adding each item one-by-one introduces high friction. However, integrating LLMs creates risks of prompt injection, untrusted external entity insertion, and upstream API cost exhaustion.
- **Decision**:
  - **Zero-Dependency OpenAI Client (`src/lib/llm/client.ts`)**: Pure `fetch` implementation with 8s `AbortSignal.timeout`, retry on transient HTTP 429/5xx errors, and strict JSON output parsing.
  - **Untrusted Ingestion Barrier (`src/lib/llm/validate.ts`)**: Model output is treated as untrusted data. String lengths are capped (`MAX_TITLE_CHARS = 300`, `MAX_CREATOR_CHARS = 300`), control characters stripped, and media types validated.
  - **Canonical Provider Verification Barrier (`findCanonicalProviderMatch`)**: Extracted candidates are never inserted directly into the circle backlog; they must match a real upstream record returned by verified media provider APIs (`Google Books`, `TMDB`, `Spotify`, `iTunes`).
  - **Durable Multi-Replica Quotas (`llm_usage`)**: PocketBase collection `llm_usage` enforces atomic fixed-slot reservations per user (5 requests/hr, 120k characters/hr) with automatic pruning of expired reservations.
- **Consequences**: Safe, friction-free unstructured text ingestion with zero provider cost overrun risk and full multi-tenant isolation.

## ADR-015: 3-Section Circle Media Lifecycle (Up Next, In Progress, Finished)
...
- **Status**: Accepted & Implemented (2026-08-23)
- **Context**: The frontend needed an editorial, tactile aesthetic matching high-quality physical publishing while maintaining clean legibility, dark mode contrast, and responsive layout fidelity.
- **Decision**:
  - **Dual Typography**: Implemented `EB Garamond` (editorial serif headings, quote snaps, and title details) alongside `Inter` (high-legibility UI controls, navigation, and badges) and `JetBrains Mono` (metadata, tags, and technical timestamps).
  - **Architectural Tokens (`src/app/globals.css`)**: Defined semantic tokens (`--color-surface-container`, `--color-primary`, `--color-accent`) with CSS custom properties ensuring crisp contrast across light and dark modes.
  - **Shared Component Primitives**: Standardized layout containers with `<AppShell>`, `<DesktopSidebar>`, and Base UI accessible dialogs.
- **Consequences**: Distinctive, cohesive visual identity with high typography readability and zero layout shifts.

## ADR-017: Session-Hoist Pattern & Relation-of-Relation Query Aggregation
- **Status**: Accepted & Implemented (2026-08-24)
- **Context**: Server Component pages often perform multiple independent data fetches in parallel. If each nested Server Action or helper calls `getSession()` and `resolveCircleAccess()`, auth refresh round-trips and DB lookups are multiplied. Furthermore, PocketBase 0.39+ does not project nested relation-of-relation data (e.g. `expand.reviews_via_title.user.*` returns bare user IDs).
- **Decision**:
