...
- **PocketBase JS Migrations Schema Gotcha**: In `pb_migrations/*.js` executed by PocketBase's Goja runtime, never use camelCase field constructor names like `new JsonField(...)` (causes `ReferenceError: JsonField is not defined`). Always use plain schema objects e.g. `{ type: "json", name: "...", required: false }`, `{ type: "bool", name: "...", required: false }`, `{ type: "text", ... }`, or all-caps Go struct equivalents (`JSONField`).
