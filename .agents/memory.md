...
- **R8 PB query capabilities (runtime-unverified)**: the group page and admin detail use nested expand-field projection (`expand.votes_via_title.*`, `expand.reviews_via_title.*`, `fields=*,expand.rel.name`) and relation dot-filters — confirmed against PB 0.27/0.39 docs but NOT runtime-tested (workers cannot run a local PB). Fallback if a deployed PB rejects them: explicit lean `getList` queries inside the same `Promise.all` (the pattern group-titles.ts already uses for own-reviews).
